...
Depending on configuration, a prompt is flagged for the following reasons:
...
>**_NOTE_**: You may face an error when you use this policy with the Gravitee's docker image because the default image is based on Alpine Linux, which does not support the ONNX Runtime. To resolve this issue, you must use the Gravitee's docker image based on Debian, which is available at `graviteeio/apim-gateway:4.8.0-debian`.
...
For more information about creating and managing resources, go to link:https://documentation.gravitee.io/apim/policies/resources[Resources]
